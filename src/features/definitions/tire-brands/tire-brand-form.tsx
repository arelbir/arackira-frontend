// Sağdan kayan modal form - lastik markası ekle/düzenle
'use client';
import React from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { tireBrandSchema, TireBrandFormValues } from './tire-brand-schema';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Sheet, SheetContent, SheetHeader, SheetTitle } from '@/components/ui/sheet';

interface TireBrandFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: TireBrandFormValues) => Promise<void>;
  initialData?: Partial<TireBrandFormValues>;
  loading: boolean;
}

const TireBrandForm: React.FC<TireBrandFormProps> = ({ open, onClose, onSubmit, initialData, loading }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<TireBrandFormValues>({
    resolver: zodResolver(tireBrandSchema),
  ...
  const submitHandler = async (data: TireBrandFormValues) => {
    await onSubmit(data);
    reset();
  };
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent side='right' className='w-full max-w-lg sm:max-w-xl md:max-w-2xl'>
        <SheetHeader>
          <SheetTitle>{initialData ? 'Markayı Düzenle' : 'Yeni Marka Ekle'}</SheetTitle>
        </SheetHeader>
