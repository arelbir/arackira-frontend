// Sağdan kayan modal form - sigorta tipi ekle/düzenle
'use client';
import React from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { insuranceTypeSchema, InsuranceTypeFormValues } from './insurance-type-schema';
...
interface InsuranceTypeFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: InsuranceTypeFormValues) => Promise<void>;
  initialData?: Partial<InsuranceTypeFormValues>;
  loading: boolean;
}

const InsuranceTypeForm: React.FC<InsuranceTypeFormProps> = ({ open, onClose, onSubmit, initialData, loading }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<InsuranceTypeFormValues>({
  ...
  const submitHandler = async (data: InsuranceTypeFormValues) => {
    await onSubmit(data);
    reset();
  };
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent side='right' className='w-full max-w-lg sm:max-w-xl md:max-w-2xl'>
        <SheetHeader>
          <SheetTitle>{initialData ? 'Sigorta Tipini Düzenle' : 'Yeni Sigorta Tipi Ekle'}</SheetTitle>
        </SheetHeader>
