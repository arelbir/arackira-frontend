// Sağdan kayan modal form - ödeme hesabı ekle/düzenle
'use client';
import React from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { paymentAccountSchema, PaymentAccountFormValues } from './payment-account-schema';
...
interface PaymentAccountFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: PaymentAccountFormValues) => Promise<void>;
  initialData?: Partial<PaymentAccountFormValues>;
  loading: boolean;
}

const PaymentAccountForm: React.FC<PaymentAccountFormProps> = ({ open, onClose, onSubmit, initialData, loading }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<PaymentAccountFormValues>({
  ...
  const submitHandler = async (data: PaymentAccountFormValues) => {
    await onSubmit(data);
    reset();
  };
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent side='right' className='w-full max-w-lg sm:max-w-xl md:max-w-2xl'>
        <SheetHeader>
          <SheetTitle>{initialData ? 'Ödeme Hesabını Düzenle' : 'Yeni Ödeme Hesabı Ekle'}</SheetTitle>
        </SheetHeader>
