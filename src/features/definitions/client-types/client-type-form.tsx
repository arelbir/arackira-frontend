// Sağdan kayan modal form - müşteri tipi ekle/düzenle
'use client';
import React from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { clientTypeSchema, ClientTypeFormValues } from './client-type-schema';
...
interface ClientTypeFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: ClientTypeFormValues) => Promise<void>;
  initialData?: Partial<ClientTypeFormValues>;
  loading: boolean;
}

const ClientTypeForm: React.FC<ClientTypeFormProps> = ({ open, onClose, onSubmit, initialData, loading }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<ClientTypeFormValues>({
  ...
  const submitHandler = async (data: ClientTypeFormValues) => {
    await onSubmit(data);
    reset();
  };
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent side='right' className='w-full max-w-lg sm:max-w-xl md:max-w-2xl'>
        <SheetHeader>
          <SheetTitle>{initialData ? 'Müşteri Tipini Düzenle' : 'Yeni Müşteri Tipi Ekle'}</SheetTitle>
        </SheetHeader>
