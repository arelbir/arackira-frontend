...
import { tyreSupplierSchema, TyreSupplierFormValues } from './tyre-supplier-schema';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Sheet, SheetContent, SheetHeader, SheetTitle } from '@/components/ui/sheet';

interface TyreSupplierFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: TyreSupplierFormValues) => Promise<void>;
  initialData?: Partial<TyreSupplierFormValues>;
  loading: boolean;
}

const TyreSupplierForm: React.FC<TyreSupplierFormProps> = ({ open, onClose, onSubmit, initialData, loading }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<TyreSupplierFormValues>({
    resolver: zodResolver(tyreSupplierSchema),
  ...
  const submitHandler = async (data: TyreSupplierFormValues) => {
    await onSubmit(data);
    reset();
  };
...
export default TyreSupplierForm;
