// Sağdan kayan modal form - vites tipi ekle/düzenle
'use client';
import React from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { transmissionSchema, TransmissionFormValues } from './transmission-schema';
...
interface TransmissionFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: TransmissionFormValues) => Promise<void>;
  initialData?: Partial<TransmissionFormValues>;
  loading: boolean;
}

const TransmissionForm: React.FC<TransmissionFormProps> = ({ open, onClose, onSubmit, initialData, loading }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<TransmissionFormValues>({
  ...
  const submitHandler = async (data: TransmissionFormValues) => {
    await onSubmit(data);
    reset();
  };
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent side='right' className='w-full max-w-lg sm:max-w-xl md:max-w-2xl'>
        <SheetHeader>
          <SheetTitle>{initialData ? 'Vites Tipini Düzenle' : 'Yeni Vites Tipi Ekle'}</SheetTitle>
        </SheetHeader>
