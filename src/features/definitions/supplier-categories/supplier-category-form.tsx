// Sağdan kayan modal form - tedarikçi kategorisi ekle/düzenle
'use client';
import React from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { supplierCategorySchema, SupplierCategoryFormValues } from './supplier-category-schema';
...
interface SupplierCategoryFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: SupplierCategoryFormValues) => Promise<void>;
  initialData?: Partial<SupplierCategoryFormValues>;
  loading: boolean;
}

const SupplierCategoryForm: React.FC<SupplierCategoryFormProps> = ({ open, onClose, onSubmit, initialData, loading }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<SupplierCategoryFormValues>({
  ...
  const submitHandler = async (data: SupplierCategoryFormValues) => {
    await onSubmit(data);
    reset();
  };
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent side='right' className='w-full max-w-lg sm:max-w-xl md:max-w-2xl'>
        <SheetHeader>
          <SheetTitle>{initialData ? 'Tedarikçi Kategorisini Düzenle' : 'Yeni Tedarikçi Kategorisi Ekle'}</SheetTitle>
        </SheetHeader>
        <form onSubmit={handleSubmit(submitHandler)} className='flex flex-col gap-4 mt-4'>
          <div>
            ...
            <Input
              ...
              disabled={loading}
              className='w-full'
              placeholder='Tedarikçi kategorisi adı'
            />
